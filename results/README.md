...
| `../repro/results/PBA/` | Theorem~\ref{thm:probe_boundary} (Appendix~\ref{app:boundary_proof}) | The empirical audit of the probe-validity boundary. `PBA_har.json` holds 405 audited HAR checkpoints --- three widths, nine doses ($\alpha \in \{0, 0.001, \ldots, 100\}$) and three seeds, each audited at epochs 5, 8, 12, 20 and 40 after a four-epoch detached warm-up. Every record carries the exact signed flux $r - \alpha c$, the probe terms $a$, $d$, $\epsilon_p$, $\epsilon_a$ and the scale $k = \mathrm{tr}(K)/(Bm)$, the frozen and live certificates with their envelopes $\Delta$ and $\Delta_{\min}$, and a float64 Richardson finite difference of $V$ along the actual gradient-flow step. `PBA_har_analysis.txt` prints the per-cell tables, the term census and the firing checkpoints. Driver: `repro/scripts/probe_boundary_audit.py` (`--selftest` checks the $K$-free identity and $\mathrm{tr}\,K$ against a dense Jacobian); reader: `repro/scripts/analyze_probe_boundary_audit.py`. The run log is not versioned (it matches the gitignored `*.log` build artefacts) but is reproduced by the driver. |
