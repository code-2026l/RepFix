...
# =============================================================================
#  Anti-collapse regularisers (self-supervised literature)
# =============================================================================
def variance_covariance_penalty(h, kind, eps=1e-4):
    """Encoder-level penalties that the SSL literature uses against collapse.

    Both act on the shared representation, i.e. on the primary objective's own
    encoder output.  They are losses, not gradient operators: nothing in the
    combination rule changes, a new term is added.

      vicreg -- VICReg's variance hinge mean_j relu(1 - std_j) plus its
        off-diagonal covariance penalty sum_{i != j} C_ij^2 / m, with the
        invariance term omitted because this protocol trains on single views
        and has no augmented pair to make invariant (Bardes et al., 2022).
      barlow -- Barlow Twins' redundancy reduction sum_ij (C_ij - d_ij)^2 / m
        on the cross-correlation matrix of the batch-standardised
        representation (Zbontar et al., 2021).
    """
    B, m = h.shape
    hc = h - h.mean(0, keepdim=True)
    ...
    if kind == "vicreg":
        ...
        cov = (hc.t() @ hc) / max(B - 1, 1)
        off = cov - torch.diag(torch.diagonal(cov))
        return var_term + off.pow(2).sum() / m
    if kind == "barlow":
        z = hc / std
        c = (z.t() @ z) / max(B - 1, 1)
        eye = torch.eye(m, device=h.device, dtype=h.dtype)
        return (c - eye).pow(2).sum() / m
    raise ValueError("unknown reg_method %r" % (kind,))
...
def train_one(domain, mode, alpha, m, seed, epochs, lr, batch, rho_c, beta,
              ...
              reg_method=None, reg_lambda=0.0):
    ...
    for ep in range(epochs):
        ...
        for b0 in range(0, n, batch):
            ...
            with amp_ctx:
                ...
                # Anti-collapse regulariser: an extra term on the shared
                # representation, added on the plain joint path.  Its weight
                # is the knob the method needs and STF does not.
                if reg_method:
                    L = L + reg_lambda * variance_covariance_penalty(h, reg_method)
...
def run_regulariser_sweep(domain, reg_methods, reg_lambdas, seeds, alpha, m,
                          epochs, lr, batch, rho_c=0.02, beta=8.0, rel_collapse=0.3):
    """Does an anti-collapse regulariser on the shared representation stop the
    toxicity transition, and at what weight and what primary-metric cost?

    Every arm is the plain joint path at supercritical alpha -- same summation
    rule, same toxic head -- with one extra loss term on the encoder output.
    The lambda grid is the point: unlike STF, these methods have a weight to
    tune, and the arm that first stops collapsing is the one to compare.
    """
    single = {s: train_one(domain, "single", 0.0, m, s, epochs, lr, batch,
                           rho_c, beta) for s in seeds}
    base = {s: single[s]["scatter"] for s in seeds}
    arms = [("joint", None, 0.0)]
    for mk in reg_methods:
        for lam in reg_lambdas:
            arms.append(("%s_l%g" % (mk, lam), mk, lam))
    recs, eff = [], []
    for label, mk, lam in arms:
        for s in seeds:
            recs.append(train_one(domain, "joint", alpha, m, s, epochs, lr, batch,
                                  rho_c, beta, base_scatter=base[s],
                                  rel_collapse=rel_collapse, method="joint",
                                  reg_method=mk, reg_lambda=lam))
            eff.append(dict(domain=domain, method=label, alpha=float(alpha), m=m, seed=s,
                            reg_method=(mk or "none"), reg_lambda=float(lam)))
    by = defaultdict(list)
    for r, e in zip(recs, eff):
        by[e["method"]].append(r)
    regress = bool(recs[0]["regress"]) if recs else False
    joint_metric = (float(np.mean([r["metric"] for r in by["joint"]])) if by.get("joint")
                    else float("nan"))
    better = lambda a, b: a < b if regress else a > b
    agg = []
    for label, mk, lam in arms:
        ...
        if not rs:
            continue
        agg.append(dict(
            domain=domain, mode=label, reg_method=(mk or "none"), reg_lambda=float(lam),
            metric_mean=float(np.mean([r["metric"] for r in rs])),
            metric_std=float(np.std([r["metric"] for r in rs])),
            collapse_rate=float(np.mean([1.0 if r["collapse"] else 0.0 for r in rs])),
            recovery_rate=(float(np.mean([1.0 if better(r["metric"], joint_metric) else 0.0
...
def main():
    ...
    ap.add_argument("--reg-methods", default="",
                    help="comma list of vicreg,barlow -> anti-collapse regulariser "
                         "sweep on the shared representation at --alphas")
    ap.add_argument("--reg-lambdas", default="0.1,1,10",
                    help="comma list of regulariser weights (the knob these "
                         "methods need and STF does not)")
    ...
    if a.composition:
        ...
        alphas = [float(x) for x in a.alphas.split(",")]
        rms = [x.strip() for x in a.reg_methods.split(",") if x.strip()]
        rls = [float(x) for x in a.reg_lambdas.split(",")]
        recs = []; agg = []; rows = []
        for al in alphas:
            _r, _a, _w = run_regulariser_sweep(a.domain, rms, rls, seeds, al, a.m,
                                               a.epochs, a.lr, a.batch, a.rho_c,
                                               a.beta, a.rel_collapse)
            for x in _a:
                x["alpha"] = al
